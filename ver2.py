...
class Interactive1DPlot:
    ...
    def redraw(self): 
        ...
        if self.title:
            self.canvas.create_text(self.width//2, 20, text=self.title,
    ...
    def on_legend_drag(self, event):
        if self.legend_dragging:
            dx = event.x - (self.legend_x + self.drag_offset_x)
            dy = event.y - (self.legend_y + self.drag_offset_y)
            self.legend_x += dx
            self.legend_y += dy
            # move all items tagged "legend"
